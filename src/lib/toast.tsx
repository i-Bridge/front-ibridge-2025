...
// Text 컴포넌트가 Tailwind 클래스를 받을 수 있다고 가정합니다.
import { Text } from '@/ui/Text'; 

// --- 1. 아이콘 정의 ---

// 아이콘: SVG 자체의 width/height는 제거하고, 
// Tailwind 클래스로 크기를 제어하도록 div로 감쌉니다.

// 1-1. 성공 아이콘
const SuccessIcon = (
  // <div className="w-6 h-6"> (기본 24px)
  <div className="w-6 h-6"> 
    <svg
      ...
      className="w-full h-full" // div의 크기를 채우도록 설정
    >
      <path
...
function FigmaToast({ icon, message }: FigmaToastProps) {
  return (
    // 토스트 본체는 'inline-flex'로 설정되어 Sonner의 중앙 정렬을 따릅니다.
    <div className="inline-flex "> 
      <div 
        ...
        "
      >
        {/* 1. 아이콘: w-6 h-6 div로 감싸져 크기가 명확해졌습니다. */}
        {icon}

        {/* 2. 통합 메시지 */}
        <Text
          variant="body02"
          // flex-1로 남은 공간을 차지하게 하고, 
          className="flex text-grayscale-gray90 " 
        >
          {message}
        </Text>
      </div>
...
// --- 3. 외부 노출 함수 (변경 없음, Sonner의 position 설정이 중요) ---
...
export const showSuccess = (message: string | ReactNode) => {
  toast.custom(
    ...
    { 
      duration: 3000, 
      // 토스트 위치를 오버라이드할 수 있지만, 
      // Toaster 컴포넌트에서 전역으로 설정하는 것이 일반적입니다.
      // position: 'top-center' // Toaster 컴포넌트 설정을 따르도록 주석 처리
    }, 
  );
};
